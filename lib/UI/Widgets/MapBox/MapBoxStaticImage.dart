...
class MapBoxStaticImage {
  ...
  // final String defaultPin = "pin-s-p+000";
  final CreatePin _defaultpin = CreatePin(
    pinColor: Colors.black,
    pinLetter: 'p',
    pinSize: 'l',
  );

  final CreatePath _defaultPath =
      CreatePath(pathColor: Colors.red, pathOpacity: 0.5, pathWidth: 5);
  ...
  String getStaticUrlWithPolyline({
    ...
    CreatePath p = path ?? _defaultPath;

    String pinUrl1 = pin1 == null
        ? _generateMarkerLink(markerUrl ?? _defaultpin.toString())
        : pin1.toString();

    String pinUrl2 = pin2 == null
        ? _generateMarkerLink(markerUrl ?? _defaultpin.toString())
        : pin2.toString();
    String url =
...
class CreatePin {
  ...
  final String pinLetter;

  CreatePin({
    this.pinColor,
    this.pinSize,
    this.pinLetter,
  })  : assert(pinColor != null),
        assert(pinLetter != null),
        assert(pinSize != null);

  @override
  String toString() {
    String color = pinColor.value.toRadixString(16).substring(2);
    String pin = "pin-$pinSize-$pinLetter+$color";
    return pin;
  }
}
